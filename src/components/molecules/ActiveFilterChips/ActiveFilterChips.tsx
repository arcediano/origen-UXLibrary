 ...
 * - Desktop (`sm:` en adelante): fila con wrap
 * - Retorna `null` si no hay chips — no ocupa espacio en el layout
 *
 * Patrón canónico de toolbar de filtros (sección 4 del manual "Bosque
 * Comercial"): se usa junto a `FilterToolbar` y `FilterSheet` en cualquier
 * listado con filtros (Productos, Pedidos, Reseñas, etc.).
 *
 * @example
 * ```tsx
 * <ActiveFilterChips
...
export interface ActiveFilterChip {
  /** Identificador único del chip (clave del filtro) */
  id: string;
  /** Texto visible del chip */
  label: string;
  /** Callback al pulsar el botón de quitar */
  onRemove: () => void;
}

export interface ActiveFilterChipsProps {
  /** Lista de filtros activos a mostrar como chips */
  chips: ActiveFilterChip[];
  /** Callback para "Limpiar todo" (solo visible si hay ≥2 chips) */
  onClearAll?: () => void;
  className?: string;
}
