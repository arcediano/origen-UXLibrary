 ...
 * @description Panel de filtros a pantalla completa para móvil/tablet, con
 * lógica de secciones tipadas, estado "draft" (cambios no se aplican hasta
 * pulsar "Ver N resultados") y footer "Limpiar filtros / Ver N resultados".
 *
 ...
 *
 * Patrón canónico (sección 4 del manual "Bosque Comercial"): se usa junto a
 * `FilterToolbar` y `ActiveFilterChips` en cualquier listado con filtros
 * (Productos, Pedidos, Reseñas, etc.). Solo se monta/renderiza en
 * móvil/tablet — en desktop los filtros equivalentes se muestran inline
 * (Select, DateInput, etc.) en el bloque `hidden lg:flex` de cada pantalla.
 *
 * @example
 * ```tsx
 * <FilterSheet
